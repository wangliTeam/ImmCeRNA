...
for(c in 1:length(cancer)){
  ...
  #max(imlnc_exp_t)  min(imlnc_exp_t)
  #[1] 3434.746         [1] 0
  #max(IRG_exp_t)    min(IRG_exp_t)
  #[1] 84347.42         [1] 0
  #max(immiR_exp_t)  min(immiR_exp_t)
  #[1] 417.0642         [1] 0
  ...
  max(corr_imlnc_IRG_r) #NA
  corr_imlnc_IRG_r[is.na(corr_imlnc_IRG_r)] <-  0
  max(corr_imlnc_IRG_r) #0.99423
  min(corr_imlnc_IRG_r) #-0.4698781
  #14:28开始34
...
#write.csv(expvali_MG_convertID,"expvali_mirTarBase_IDconvert.csv")
  ...
  expvali_miRgene_app <- apply(ceRNA_app_data,1,expvali_miRgene) #TRUE or FALSE 值
  expvali_miRgene_data <- ceRNA_app_data[expvali_miRgene_app,]
  dim(expvali_miRgene_data)
  #[1][1] 4040    3
  
  mylist[["expvaliImCe"]]["miRgene"] <- paste(cancer[c],"中实验证实的miRgene关系对的数目为：",dim(expvali_miRgene_data)[1],";其中lncRNA_num:",length(unique(expvali_miRgene_data[,1])),";miRNA_num:",length(unique(expvali_miRgene_data[,2])),";gene_num:",length(unique(expvali_miRgene_data[,3])),sep="")
  ...
  mylist[["expvaliImCe"]]["lncmiR"] <- paste(cancer[c],"中实验证实的lncmiR关系对的数目为：",dim(expvali_lncmiR_data)[1],paste(";lncRNA_num:",length(unique(expvali_lncmiR_data[,1])),";miRNA_num:",length(unique(expvali_lncmiR_data[,2])),";gene_num:",length(unique(expvali_lncmiR_data[,3])),sep=""),sep="")
  ...
  mylist[["expvaliImCe"]]["ceRNA"] <- paste(cancer[c],"中实验证实的关系对的数量为：",dim(expvali_ceRNA_data)[1],paste("  lncRNA_num:",length(unique(expvali_ceRNA_data[,1])),"  miRNA_num:",length(unique(expvali_ceRNA_data[,2])),"  gene_num:",length(unique(expvali_ceRNA_data[,3])),sep=""),sep="")
  print(cancer[c])
  print(mylist)
